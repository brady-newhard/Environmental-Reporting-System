...
const SWPPPPhotoPage = () => {
  ...
  useEffect(() => {
    const fetchPhotos = async () => {
      try {
        const response = await api.get(`/swppp/reports/${reportId}/photos/`);
  ...
  const handleUpload = async () => {
    ...
    try {
      await api.post(`/swppp/reports/${reportId}/photos/`, formData, {
  ...
  const handleDelete = async (photoId) => {
    try {
      await api.delete(`/swppp/reports/${reportId}/photos/${photoId}/`);
