...
const SWPPPReportSubmitted = () => {
  ...
  useEffect(() => {
    const loadSubmittedReports = async () => {
      try {
        setLoading(true);
        const reports = await getSubmittedReports({ report_type: 'swppp' });
  ...
  const handleEditReport = (report) => {
    ...
    navigate(`/environmental/swppp/edit-submitted/${report.id}`, {
      state: { 
  ...
  const handleViewReport = (report) => {
    ...
    navigate(`/environmental/reports/swppp/print/${report.id}`, {
      state: { 
        reportData: report.report_data,
        reportId: report.id,
        isSubmitted: true
        ...
        <div className="bg-gray-800 shadow rounded-lg border border-gray-700">
          ...
          {submittedReports.length === 0 ? (
            <div className="px-6 py-12 text-center">
              ...
              <Button 
                onClick={() => navigate('/environmental/swppp/new')}
                  ...
                  {submittedReports.map((report) => (
                    <tr key={report.id} className="hover:bg-gray-700">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">
                        {report.report_id || `SWPPP-${report.id}`}
                      </td>
...
export default SWPPPReportSubmitted;
