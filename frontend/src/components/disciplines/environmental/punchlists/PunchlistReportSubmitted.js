...
const PunchlistReportSubmitted = () => {
  ...
  useEffect(() => {
    const loadSubmittedReports = async () => {
      try {
        setLoading(true);
        const reports = await getSubmittedReports({ report_type: 'punchlist' });
  ...
  const handleEditReport = (report) => {
    ...
    navigate(`/environmental/reports/punchlist/edit-submitted/${report.id}`, {
      state: { 
  ...
  const handleViewReport = (report) => {
    ...
    navigate(`/environmental/reports/punchlist/print/${report.id}`, {
      state: { 
        reportData: report.report_data,
        reportId: report.id,
        isSubmitted: true
  ...
  return (
    <div className="min-h-screen bg-black">
      <PageHeader 
        title="Submitted Punchlist Reports" 
        ...
        <div className="bg-gray-800 shadow rounded-lg border border-gray-700">
          ...
          {submittedReports.length === 0 ? (
            <div className="px-6 py-12 text-center">
              ...
              <Button 
                onClick={() => navigate('/environmental/reports/punchlist/new')}
              ...
              <table className="min-w-full divide-y divide-gray-700">
                ...
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {submittedReports.map((report) => {
                    const punchlistItems = report.report_data?.punchlist_items || [];
                    return (
                      <tr key={report.id} className="hover:bg-gray-700">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">
                          {report.report_id || `Punchlist-${report.id}`}
                        </td>
...
export default PunchlistReportSubmitted;
