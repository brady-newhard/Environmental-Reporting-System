...
const DailyWeldingReportStationReview = () => {
  ...
  const backPath = location.state?.from || '/welding/reports/station-drafts';
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <PageHeader title="Daily Weld Station Report Review" backPath={backPath} />
      <Paper sx={{ p: 3, bgcolor: '#fff' }}>
        <Typography variant="h4" gutterBottom>Daily Weld Station Report</Typography>
        {/* Project Info */}
        ...
        {sectionTitle('Cut Out Defects in Pipe')}
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 4, mb: 2 }}>
          <Box><b>2 Cuts:</b> Size: {form.cutOut2?.size}, Defect: {form.cutOut2?.defect}</Box>
          <Box><b>1 Cut:</b> Size: {form.cutOut1?.size}, Defect: {form.cutOut1?.defect}</Box>
        </Box>
        {/* Signatures */}
        ...
        <Stack direction="row" spacing={2} sx={{ mt: 3 }}>
          <Button variant="outlined" onClick={() => navigate('/welding/reports/daily-station', { state: { draft: form } })}>Edit</Button>
...
export default DailyWeldingReportStationReview; 