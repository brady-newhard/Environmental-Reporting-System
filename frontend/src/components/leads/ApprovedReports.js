...
const ApprovedReports = () => {
  ...
  const fetchReports = async () => {
    try {
      const response = await api.get('/api/reports/?status=approved');
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Approved Reports</h2>
      ...
      {filteredReports.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-600">No approved reports found.</p>
        </div>
      ) : (
