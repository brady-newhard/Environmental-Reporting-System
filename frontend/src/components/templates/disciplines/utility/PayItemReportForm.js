...
const PayItemReportForm = () => {
  ...
  useEffect(() => {
    const loadExistingDraft = async () => {
      if (id) {
        try {
          const loadedDraft = await loadDraft('pay_item', id);
          if (loadedDraft) {
  ...
  const handleSave = async (formData) => {
    try {
      ...
      const savedDraft = await saveDraft('pay_item', dataToSave);
      
      setSnackbar({ show: true, message: 'Draft saved successfully', severity: 'success' });
      
      if (!draftId) {
        setDraftId(savedDraft.id);
        navigate(`/utility/reports/pay-item/edit/${savedDraft.id}`, { replace: true });
      }
  ...
  const handleReview = () => {
    navigate(`/utility/reports/pay-item/review/${draftId}`, {
      state: { 
        header, 
        items, 
        comments, 
        ...
        <form onSubmit={handleFormSubmit} className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-md p-4 mb-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl md:text-2xl font-bold text-gray-800">Pay Items</h2>
              <button
                type="button"
...
export default PayItemReportForm; 